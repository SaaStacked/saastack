...
public class MigrationsApiSpec
{
    ...
    public class GivenSimpleBillingProvider : WebApiSpec<Program>
    {
        ...
        public async Task WhenExportSubscriptionsToBeMigrated_ThenReturnsSubscriptions()
        {
            ...
            result[0].Buyer[nameof(SubscriptionBuyer.Subscriber)].Should()
                .Be(
                    $"{{\"EntityId\":\"{login.DefaultOrganizationId}\",\"EntityType\":\"Organization\",\"EntityName\":\"persona alastname\"}}");
    ...
    [Trait("Category", "Integration.API")]
    [Collection("API")]
    public class GivenAFakeBillingProvider : WebApiSpec<Program>
    {
        public GivenAFakeBillingProvider(WebApiSetup<Program> setup) : base(setup, OverrideDependencies)
        {
            EmptyAllRepositories();
        }
