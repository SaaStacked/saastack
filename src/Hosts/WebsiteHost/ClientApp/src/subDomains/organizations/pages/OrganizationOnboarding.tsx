...
import {
  ...
  InitiateOnboardingWorkflowResponse,
...
import { useFormatters } from '../../../framework/i18n/formatters.ts';
...
import { customWorkflow, shouldBeOnboarding } from './Onboarding.ts';
    ...
    if (shouldBeOnboarding(organization)) {
      switch (organization!.onboardingStatus) {
        case OrganizationOnboardingStatus.NOT_STARTED:
          if (hasTriggeredInitialLoad.current) {
            return;
          }
  ...
  return (
    <FormPage title={translate('pages.organizations.onboarding.title')}>
      <PageAction
        ...
        loadingMessage={translate('pages.organizations.onboarding.loaders.initiating')}
        onSuccess={(_: {
          requestData?: InitiateOnboardingWorkflowRequest;
          response: InitiateOnboardingWorkflowResponse;
        }) => getOnboardingTrigger.current?.execute()}
      >
        <p className="text-neutral-500 dark:text-neutral-400 text-xs">
          {translate('pages.organizations.onboarding.states.initiated', { time: formatTime(new Date()) })}
        </p>
      </PageAction>
      ...
      <PageAction
        ...
        loadingMessage={translate('pages.organizations.onboarding.loaders.loading')}
      >
...
function OnboardingWorkflow({ workflow }: OnboardingWorkflowProps) {
  ...
  const currentStepTranslationKey = `pages.organizations.onboarding.steps.${currentStep.id.replace(/\./g, '_')}`;
    ...
    <div className="w-full max-w-4xl mx-auto" data-testid="onboarding_workflow">
      ...
      >
        <h2 className="text-2xl font-bold mb-4 text-neutral-900 dark:text-neutral-100">
          {translate(`${currentStepTranslationKey}.title`)}
        </h2>
        <p className="text-neutral-700 dark:text-neutral-300 mb-6">
          {translate(`${currentStepTranslationKey}.description`)}
        </p>
      </div>
